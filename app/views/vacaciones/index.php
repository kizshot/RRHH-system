<?php
// app/views/vacaciones/index.php - Listado y modal Agregar Vacaciones
...
$statuses = ['PENDIENTE','APROBADO','RECHAZADO','EN_CURSO','COMPLETADO'];
?>

<style>
/* Estilos mejorados para el módulo de Vacaciones */
.vacaciones-header {
...
<div class="vacaciones-header">
  <h2><i class="fa fa-umbrella-beach"></i> Gestión de Vacaciones</h2>
  <p>Administra las solicitudes de vacaciones de los empleados</p>
</div>
...
<div class="search-container">
  <form method="get" action="/Recursos/index.php" class="search-form">
    <input type="hidden" name="route" value="vacaciones.index">
    ...
    <select class="input" name="sort">
      <?php $sorts = ['id'=>'ID','personal_id'=>'Empleado','start_date'=>'Fecha Inicio','end_date'=>'Fecha Fin','days_requested'=>'Días','status'=>'Estado']; foreach($sorts as $k=>$label): ?>
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
  <h3 style="margin: 0; color: #374151;">Lista de Vacaciones</h3>
          ...
          <td>
            <div class="employee-info">
              <div class="employee-avatar">
                <?= strtoupper(substr($v['employee_first_name'] ?? '', 0, 1) . substr($v['employee_last_name'] ?? '', 0, 1)) ?>
              </div>
              <div class="employee-details">
                <h4><?= htmlspecialchars(trim(($v['employee_first_name'] ?? '').' '.($v['employee_last_name'] ?? ''))) ?></h4>
                <small><?= htmlspecialchars($v['employee_code'] ?? '') ?></small>
              </div>
            </div>
          </td>
          <td>
            <div class="dates-info">
              <div class="dates-value"><?= htmlspecialchars($v['start_date'] ?? '') ?> - <?= htmlspecialchars($v['end_date'] ?? '') ?></div>
              <div class="dates-label">Período</div>
            </div>
          </td>
          <td>
            <div class="days-info">
              <div class="days-value"><?= (int)($v['days_requested'] ?? 0) ?> días</div>
              <div class="days-label">Solicitados</div>
              <?php if($v['days_approved']): ?>
                <br><small style="color: #6b7280;">Aprobados: <?= (int)$v['days_approved'] ?></small>
              <?php endif; ?>
              <?php if($v['days_taken']): ?>
                <br><small style="color: #6b7280;">Tomados: <?= (int)$v['days_taken'] ?></small>
              <?php endif; ?>
            </div>
          </td>
          <td>
            <span class="status-badge <?= strtolower($v['status']) === 'pendiente' ? 'status-pendiente' : (strtolower($v['status']) === 'aprobado' ? 'status-aprobado' : (strtolower($v['status']) === 'rechazado' ? 'status-rechazado' : (strtolower($v['status']) === 'en_curso' ? 'status-en_curso' : 'status-completado'))) ?>">
              <?= htmlspecialchars($v['status']) ?>
            </span>
          </td>
          <td>
            <div class="action-buttons">
              <button class="btn-action btn-view" onclick="openViewModal(<?= (int)$v['id'] ?>)" title="Ver detalles">
                <i class="fa fa-eye"></i>
              </button>
              <button class="btn-action btn-edit" onclick="openEditModal(<?= (int)$v['id'] ?>)" title="Editar">
                <i class="fa fa-pen"></i>
              </button>
              <?php if(strtolower($v['status']) === 'pendiente'): ?>
                <button class="btn-action btn-approve" onclick="approveVacacion(<?= (int)$v['id'] ?>)" title="Aprobar">
                  <i class="fa fa-check"></i>
                </button>
                <button class="btn-action btn-reject" onclick="rejectVacacion(<?= (int)$v['id'] ?>)" title="Rechazar">
                  <i class="fa fa-times"></i>
                </button>
              <?php elseif(strtolower($v['status']) === 'aprobado'): ?>
                <button class="btn-action btn-start" onclick="startVacacion(<?= (int)$v['id'] ?>)" title="Iniciar vacaciones">
                  <i class="fa fa-play"></i>
                </button>
              <?php elseif(strtolower($v['status']) === 'en_curso'): ?>
                <button class="btn-action btn-complete" onclick="completeVacacion(<?= (int)$v['id'] ?>)" title="Completar vacaciones">
                  <i class="fa fa-flag-checkered"></i>
                </button>
              <?php endif; ?>
              <button class="btn-action btn-delete" onclick="confirmDelete(<?= (int)$v['id'] ?>, '<?= htmlspecialchars($v['employee_first_name'] . ' ' . $v['employee_last_name']) ?>')" title="Eliminar">
...
<div class="pagination">
  ...
  <?php if (($page ?? 1) > 1): ?>
    <a href="/Recursos/index.php?route=vacaciones.index&page=<?= ($page-1) . $qparam ?>">
...
<!-- Modal crear vacaciones mejorado -->
<div class="modal" id="modal-create" role="dialog" aria-modal="true" aria-labelledby="modalTitle" style="display:none">
  ...
  <form id="create-form" action="/Recursos/index.php?route=vacaciones.create" method="post">
    <div class="modal-body">
      <div class="grid-2">
        ...
        <div class="form-row">
          <label for="days_requested">Días Solicitados</label>
          <input class="input" type="number" id="days_requested" name="days_requested" min="1" max="30">
        </div>
        <div class="form-row">
          <label for="days_approved">Días Aprobados</label>
          <input class="input" type="number" id="days_approved" name="days_approved" min="0" max="30">
        </div>
        <div class="form-row">
          <label for="days_taken">Días Tomados</label>
          <input class="input" type="number" id="days_taken" name="days_taken" min="0" max="30">
        </div>
        <div class="form-row">
        ...
        <div class="form-row">
          <label for="start_vacation_date">Fecha Real de Inicio</label>
          <input class="input" type="date" id="start_vacation_date" name="start_vacation_date">
        </div>
        <div class="form-row">
          <label for="end_vacation_date">Fecha Real de Fin</label>
          <input class="input" type="date" id="end_vacation_date" name="end_vacation_date">
        </div>
...
<div class="modal" id="modal-delete" role="dialog" aria-modal="true" style="display:none; max-width: 500px;">
  ...
  <div class="modal-body" style="text-align: center; padding: 2rem;">
    ...
    <p id="delete-message" style="font-size: 1.1rem; margin-bottom: 1rem;">¿Estás seguro de que quieres eliminar estas vacaciones?</p>
...
<script src="/Recursos/assets/js/main.js"></script>
      ...
      function confirmDelete(id, name) {
        ...
        document.getElementById('delete-message').innerHTML = 
          '¿Estás seguro de que quieres eliminar las vacaciones de <strong>' + name + '</strong>?';
      ...
      function executeDelete() {
        if (deleteId && deleteName) {
          ...
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=vacaciones.delete';
      ...
      // Funciones para aprobar, rechazar, iniciar y completar vacaciones
      function approveVacacion(id) {
        if (confirm('¿Estás seguro de que quieres aprobar estas vacaciones?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=vacaciones.approve';
      ...
      function rejectVacacion(id) {
        if (confirm('¿Estás seguro de que quieres rechazar estas vacaciones?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=vacaciones.reject';
      ...
      function startVacacion(id) {
        if (confirm('¿Estás seguro de que quieres iniciar estas vacaciones?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=vacaciones.startVacation';
      ...
      function completeVacacion(id) {
        if (confirm('¿Estás seguro de que quieres completar estas vacaciones?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=vacaciones.completeVacation';
      ...
      (function(){
        ...
        if (createForm) {
          createForm.addEventListener('submit', function(e) {
            ...
            fetch('/Recursos/index.php?route=vacaciones.create', {
              method: 'POST',
              headers: {
            ...
            .then(data => {
              if (data.success) {
                ...
                // Mostrar error
                alert('Error al crear vacaciones: ' + data.message);
              }
            })
            .catch(error => {
              alert('Error: ' + error);
      ...
      function openViewModal(id) {
        fetch('/Recursos/index.php?route=vacaciones.view&id=' + id)
          .then(response => response.text())
          .then(data => {
            document.getElementById('modal-content').innerHTML = data;
            document.getElementById('modal-title').textContent = 'Ver Vacaciones';
      ...
      function openEditModal(id) {
        fetch('/Recursos/index.php?route=vacaciones.edit&id=' + id)
          .then(response => response.text())
          .then(data => {
            document.getElementById('modal-content').innerHTML = data;
      ...
      // Función global para editar vacaciones (usada desde modal de vista)
      function editVacacion(id) {
