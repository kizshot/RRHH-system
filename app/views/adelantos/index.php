<?php
// app/views/adelantos/index.php - Listado y modal Agregar Adelantos
...
<style>
/* Estilos mejorados para el módulo de Adelantos */
.adelantos-header {
...
<div class="adelantos-header">
  <h2><i class="fa fa-money-bill-wave"></i> Gestión de Adelantos</h2>
  <p>Administra los adelantos de salario solicitados por los empleados</p>
</div>
...
<div class="search-container">
  <form method="get" action="/Recursos/index.php" class="search-form">
    <input type="hidden" name="route" value="adelantos.index">
    ...
    <select class="input" name="sort">
      <?php $sorts = ['id'=>'ID','personal_id'=>'Empleado','amount'=>'Monto','request_date'=>'Fecha Solicitud','status'=>'Estado']; foreach($sorts as $k=>$label): ?>
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
  <h3 style="margin: 0; color: #374151;">Lista de Adelantos</h3>
  <button class="btn-add" id="btn-open-modal">
    <i class="fa fa-plus"></i> Solicitar Adelanto
  </button>
</div>
        ...
        <tr>
          ...
          <td>
            <div class="amount-info">
              <div class="amount-value">S/ <?= number_format($a['amount'], 2) ?></div>
              <div class="amount-label">Adelanto</div>
            </div>
          </td>
          <td>
            <div style="max-width: 200px;">
              <strong><?= htmlspecialchars($a['reason'] ?? 'Sin motivo') ?></strong>
              <?php if($a['notes']): ?>
                <br><small style="color: #6b7280;"><?= htmlspecialchars($a['notes']) ?></small>
              <?php endif; ?>
            </div>
          </td>
          <td>
            <span class="status-badge <?= strtolower($a['status']) === 'pendiente' ? 'status-pendiente' : (strtolower($a['status']) === 'aprobado' ? 'status-aprobado' : (strtolower($a['status']) === 'rechazado' ? 'status-rechazado' : 'status-pagado')) ?>">
              <?= htmlspecialchars($a['status']) ?>
            </span>
          </td>
          <td>
            <span style="font-weight: 500;"><?= htmlspecialchars($a['request_date'] ?? '') ?></span>
            <?php if($a['approved_date']): ?>
              <br><small style="color: #6b7280;">Aprobado: <?= htmlspecialchars($a['approved_date']) ?></small>
            <?php endif; ?>
          </td>
          <td>
            <div class="action-buttons">
              ...
              <?php if(strtolower($a['status']) === 'pendiente'): ?>
                <button class="btn-action btn-approve" onclick="approveAdelanto(<?= (int)$a['id'] ?>)" title="Aprobar">
                  <i class="fa fa-check"></i>
                </button>
                <button class="btn-action btn-reject" onclick="rejectAdelanto(<?= (int)$a['id'] ?>)" title="Rechazar">
                  <i class="fa fa-times"></i>
                </button>
              <?php elseif(strtolower($a['status']) === 'aprobado'): ?>
                <button class="btn-action btn-paid" onclick="markAsPaid(<?= (int)$a['id'] ?>)" title="Marcar como pagado">
...
<div class="pagination">
  ...
  <?php if (($page ?? 1) > 1): ?>
    <a href="/Recursos/index.php?route=adelantos.index&page=<?= ($page-1) . $qparam ?>">
...
<div class="modal" id="modal-create" role="dialog" aria-modal="true" aria-labelledby="modalTitle" style="display:none">
  ...
  <form id="create-form" action="/Recursos/index.php?route=adelantos.create" method="post">
    <div class="modal-body">
      <div class="grid-2">
        ...
        <div class="form-row">
          ...
          <input class="input" type="number" id="amount" name="amount" step="0.01" min="0" required>
        </div>
        <div class="form-row">
          <label for="request_date">Fecha de Solicitud *</label>
          <input class="input" type="date" id="request_date" name="request_date" required>
        </div>
        <div class="form-row">
        ...
        <div class="form-row" style="grid-column: 1 / -1;">
          ...
          <textarea class="input" id="reason" name="reason" rows="3" required></textarea>
        </div>
        <div class="form-row" style="grid-column: 1 / -1;">
...
<div class="modal" id="modal-delete" role="dialog" aria-modal="true" style="display:none; max-width: 500px;">
  ...
  <div class="modal-body" style="text-align: center; padding: 2rem;">
    ...
    <p id="delete-message" style="font-size: 1.1rem; margin-bottom: 1rem;">¿Estás seguro de que quieres eliminar este adelanto?</p>
...
<script src="/Recursos/assets/js/main.js"></script>
      ...
      function confirmDelete(id, name) {
        ...
        document.getElementById('delete-message').innerHTML = 
          '¿Estás seguro de que quieres eliminar el adelanto de <strong>' + name + '</strong>?';
      ...
      function executeDelete() {
        if (deleteId && deleteName) {
          ...
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=adelantos.delete';
      ...
      function approveAdelanto(id) {
        if (confirm('¿Estás seguro de que quieres aprobar este adelanto?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=adelantos.approve';
      ...
      function rejectAdelanto(id) {
        if (confirm('¿Estás seguro de que quieres rechazar este adelanto?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=adelantos.reject';
      ...
      function markAsPaid(id) {
        if (confirm('¿Estás seguro de que quieres marcar este adelanto como pagado?')) {
          var form = document.createElement('form');
          form.method = 'POST';
          form.action = '/Recursos/index.php?route=adelantos.markAsPaid';
      ...
      (function(){
        ...
        if (createForm) {
          createForm.addEventListener('submit', function(e) {
            ...
            fetch('/Recursos/index.php?route=adelantos.create', {
              method: 'POST',
              headers: {
            ...
            .then(data => {
              if (data.success) {
                ...
                // Mostrar error
                alert('Error al crear adelanto: ' + data.message);
              }
            })
            .catch(error => {
              alert('Error: ' + error);
      ...
      function openViewModal(id) {
        fetch('/Recursos/index.php?route=adelantos.view&id=' + id)
          .then(response => response.text())
          .then(data => {
            document.getElementById('modal-content').innerHTML = data;
            document.getElementById('modal-title').textContent = 'Ver Adelanto';
      ...
      // Función global para editar adelanto (usada desde modal de vista)
